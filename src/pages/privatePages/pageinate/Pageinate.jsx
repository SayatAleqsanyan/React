...
const PaginatedItems = () => {
  ...
  const mockItems = Array.from({ length: 200 }, (_, index) => ({
    id: index + 1,
    title: `Item ${index + 1} Title`,
  }))


  const pageCount = Math.ceil(mockItems.length / itemsPerPage)
        ...
        <div className="space-y-4">
          {currentItems.map(item => (
            <div key={item.id} className="border p-4 rounded">
              <h3 className="text-lg font-semibold">{item.id}. Item</h3>
